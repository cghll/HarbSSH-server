package com.gduf.domain.agent.service.intent;

import com.gduf.domain.agent.model.valobj.intent.ConversationContextVO;
...
import com.gduf.domain.agent.service.intent.classifier.impl.LLMIntentClassifier;
import com.gduf.domain.agent.service.intent.classifier.impl.RuleIntentClassifier;
...
public class IntentService implements IIntentService {
    ...
    private final Map<String, CacheEntry> cache = Collections.synchronizedMap(
            ...
            new LinkedHashMap<String, CacheEntry>(16, 0.75f, true) {
                ...
                //put 新增元素之后，会自动调用这个方法,最大只能存 200 条，超过就触发 LRU 淘汰,缓存满了就删链表尾部
                @Override
    ...
    public IntentResultVO classify(String sessionId, String userId, String message) {
        ...
        // 2）连续失败次数过多：跳过规则层，直接 LLM 兜底，门槛放宽
