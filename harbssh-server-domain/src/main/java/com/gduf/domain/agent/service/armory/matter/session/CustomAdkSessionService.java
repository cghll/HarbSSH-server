...
import com.gduf.domain.agent.service.armory.matter.session.model.SessionSnapshot;
...
public class CustomAdkSessionService implements BaseSessionService {
     ...
     *
     *   创建后得到：
     *   sessions["harbssh-app"]["user-001"]["react-session-01"] = SessionSnapshot
     * </pre>
     *
     * @param appName 应用名
     * @param userId 用户 ID
     ...
     */
    @Override
    public Single<Session> createSession(String appName, String userId, @Nullable ConcurrentMap<String, Object> initialState, @Nullable String sessionId) {
    ...
    public Single<Event> appendEvent(Session session, Event event) {
        ...
        SessionSnapshot snapshot = findSnapshot(key.appName(), key.userId(), session.id());
        if (snapshot == null) {
