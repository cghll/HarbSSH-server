...
import com.gduf.cases.react.node.RootNode;
...
public class AIAgentReActServiceCase implements IAIAgentReActServiceCase {
    ...
    public ResponseBodyEmitter chatStream(ChatRequestDTO requestDTO) {
        ...
        try {
            ...
            //2.舒适化上下文
            DefaultReActFactory.DynamicContext dynamicContest = DefaultReActFactory.DynamicContext.builder()
                    .emitter(emitter)
                    .build();

            //3.异步执行节点链路（避免阻塞HTTP线程）
            Thread streamThread = getThread(requestDTO, dynamicContest, emitter);
            streamThread.start();
        } catch (Exception e) {
