package com.gduf.trigger;


import com.gduf.api.dto.ChatRequestDTO;
import com.gduf.cases.IAIAgentReActServiceCase;
...
import com.gduf.domain.ssh.model.entity.TerminalSessionEntity;
...
public class SshAgentReActTest {
    ...
    // 使用方法：复制 src/test/resources/ssh-config.local 为 ssh-config.local，填入你的服务器信息
    ...
    private static final String USER_ID = "react-cghry";
    // =================================================================

    /**
     * 从 classpath:ssh-config.local 读取 SSH 连接配置
     * <p>
     * 该文件不提交到 Git（已在 .gitignore 中排除），使用前请从 ssh-config.local 复制并填写真实信息。
     */
        ...
        try (InputStream is = SshAgentReActTest.class.getClassLoader().getResourceAsStream("ssh-config.local")) {
            if (is == null) {
                throw new RuntimeException(
                        "未找到 ssh-config.local 文件！请复制 src/test/resources/ssh-config.local 为 ssh-config.local 并填写服务器信息。");
            }
            props.load(is);
        } catch (Exception e) {
            throw new RuntimeException("加载 ssh-config.local 失败: " + e.getMessage(), e);
        }
        return props;
    }
